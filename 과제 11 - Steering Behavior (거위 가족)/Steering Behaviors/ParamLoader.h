...
private:
	ParamLoader() :iniFileLoaderBase("params.ini")
	{
		...
		CatchingWeight = GetNextParameterFloat() * SteeringForceTweaker;
		CatchedWeight = GetNextParameterFloat() * SteeringForceTweaker;
		...
		prCatching = GetNextParameterFloat();
		prCatched = GetNextParameterFloat();
...
public:
	...
	double CatchingWeight;
	double CatchedWeight;
