...
SteeringBehavior::SteeringBehavior(Vehicle* agent) :
	...
	m_dWeightCatching(Prm.CatchingWeight),
	m_dWeightCatched(Prm.CatchedWeight),
...
Vector2D SteeringBehavior::Calculate()
{
	...
	m_vSteeringForce.Zero(); // 우선, 조정 힘을 0으로 만든다.

	if (On(separation) || On(allignment) || On(cohesion)) // 만약 이 3가지 행동 중 하나를 진행 중일때,
	{
		if (!isSpacePartitioningOn()) // 공간 분할을 사용하지 않는다면, 현 객체의 이웃 객체들(시야에 들어온 객체들)에 태그를 건다.
		{
			m_pVehicle->World()->TagVehiclesWithinViewRange(m_pVehicle, m_dViewDistance);
		}
		else // 공간 분할을 사용한다면, 셀 공간 내부의 이웃 객체들을 계산한다.
		{
			m_pVehicle->World()->CellSpace()->CalculateNeighbors(m_pVehicle->Pos(), m_dViewDistance);
		}
	}

	// 행동 변화에 따라 가해질 힘의 조합함에 있어서,
	// 3가지 방법 중 하나를 사용하게된다.
	switch (m_SummingMethod)
	{
	case weighted_average: m_vSteeringForce = CalculateWeightedSum(); break;
	case prioritized: m_vSteeringForce = CalculatePrioritized(); break;
	case dithered: m_vSteeringForce = CalculateDithered(); break;
...
bool SteeringBehavior::AccumulateForce(Vector2D &RunningTot, Vector2D ForceToAdd)
{
	...
	if (MagnitudeRemaining <= 0.0) return false; // 남은 힘이 없다면 return false

	// 가해줄 힘을 계산한다.
	double MagnitudeToAdd = ForceToAdd.Length();

	// 남아있는 힘이 가해줄 힘 보다 크다면 그냥 '힘을 가해준다'.
	if (MagnitudeToAdd < MagnitudeRemaining) RunningTot += ForceToAdd;
	else RunningTot += (Vec2DNormalize(ForceToAdd) * MagnitudeRemaining); // 아니라면, '가해줄 힘의 방향'으로 '남아있는 힘의 크기' 만큼 힘을 가해준다.
...
{
	// Weighted Sum 방식
	// => 각 활성화된 행동들에 각 가중치를 곱한 값을 조정 힘에 '모두 더한다'.
	// => 단순하지만, 상충되는 힘의 조정이 어렵다.

	if (On(wall_avoidance)) m_vSteeringForce += WallAvoidance(m_pVehicle->World()->Walls()) * m_dWeightWallAvoidance;
	if (On(obstacle_avoidance)) m_vSteeringForce += ObstacleAvoidance(m_pVehicle->World()->Obstacles()) * m_dWeightObstacleAvoidance;
	...
	if (On(catching))
	{
		assert(m_pTargetAgent1 && "Catching target not assigned");
		m_vSteeringForce += Catched(m_pTargetAgent1) * m_dWeightCatching;
	}

	if (On(catched))
	{
		assert(m_pTargetAgent1 && "Catched target not assigned");
		m_vSteeringForce += Catched(m_pTargetAgent1) * m_dWeightCatched;
	}
...
Vector2D SteeringBehavior::CalculatePrioritized()
{
	...
	{
		assert(m_pTargetAgent1 && "Catching target not assigned");
		force = Catching(m_pTargetAgent1) * m_dWeightCatching;
		if (!AccumulateForce(m_vSteeringForce, force)) return m_vSteeringForce;
	}
	...
	{
		assert(m_pTargetAgent1 && "Catched target not assigned");
		force = Catched(m_pTargetAgent1) * m_dWeightCatched;
		if (!AccumulateForce(m_vSteeringForce, force)) return m_vSteeringForce;
	}
...
Vector2D SteeringBehavior::CalculateDithered()
{
	...
	if (On(catched) && RandFloat() < Prm.prCatching)
	{
		assert(m_pTargetAgent1 && "Catched target not assigned");
		m_vSteeringForce += Catching(m_pTargetAgent1) * m_dWeightCatching / Prm.prCatching;
	...
	if (On(catched) && RandFloat() < Prm.prCatched)
	{
		assert(m_pTargetAgent1 && "Catched target not assigned");
		m_vSteeringForce += Catched(m_pTargetAgent1) * m_dWeightCatched / Prm.prCatched;
...
Vector2D SteeringBehavior::Flee(Vector2D TargetPos)
{
	...
	if (Vec2DDistanceSq(m_pVehicle->Pos(), TargetPos) > PanicDistanceSq) return Vector2D(0, 0);
...
{
	Vector2D ToTarget = TargetPos - m_pVehicle->Pos(); // 타겟으로 향하는 벡터를 구한다.
	double dist = ToTarget.Length(); // 구한 벡터의 길이를 구한다.
	if (dist > 0) // 길이가 0 보다 크다면,
	{
		const double DecelerationTweaker = 0.2; // 감속 조정값
		double speed = dist / ((double)deceleration * DecelerationTweaker); // 감속된 속도를 구한다.
		speed = min(speed, m_pVehicle->MaxSpeed()); // 구한 속도 값이 최대 속도 값을 넘지 않게 조정한다.

		Vector2D DesiredVelocity = ToTarget * speed / dist; // 현 위치에서 목표를 향한 벡터를 구한다.

		return (DesiredVelocity - m_pVehicle->Velocity()); // 가해져야 할 벡터(현재 객체에 가해지는 벡터에서 목표로 향하는 벡터)를 구해서 리턴한다.
	}

	return Vector2D(0, 0); // 길이가 0 보다 작다면, 0 벡터를 리턴. (아무런 힘을 가하지 않음)
}
...
Vector2D SteeringBehavior::Catching(const Vehicle* pursuer)
{
	// ** 추격자에게 잡혔는지 검사한다. **
	Vector2D ToPursuer = pursuer->Pos() - m_pVehicle->Pos(); // 추격자로 향하는 벡터를 구한다.

	const double ThreatRange = 70.0; // 위협 범위
	if (ToPursuer.LengthSq() > ThreatRange * ThreatRange) return Vector2D(); // 위협 범위 밖에 있다면, 0 벡터를 리턴. (아무런 힘을 가하지 않음)

	WanderOff(); // 범위 안에 있다면, 배회를 멈춘다.
	CatchingOff(); // 더이상 잡혔는지를 검사하면 안된다.
	CatchedOn(); // 그리고 '잡힘'을 켠다.
	m_pVehicle->Set_is_Catched(true);
	m_pTargetAgent1 = m_pVehicle->Get_Last_Vehicle(); // 마지막에 붙은 객체를 타겟으로 한다.
	m_pVehicle->Set_Last_Vehicle(m_pVehicle); // 자신이 마지막 객체라는것을 알려주어야한다. (수정)
	return Arrive(m_pTargetAgent1->Pos(), slow); // Arrive를 수행.
}

Vector2D SteeringBehavior::Catched(const Vehicle* pursuer)
{
	// ** 추격자에게 잡혔을 때, **
	Vector2D ToPursuer = pursuer->Pos() - m_pVehicle->Pos(); // 추격자로 향하는 벡터를 구한다.
	return Arrive(pursuer->Pos(), fast); // Arrive를 수행.
}
