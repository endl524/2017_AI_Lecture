...
void Visit_Bank_And_Deposit_Gold::Execute(Miner* pMiner)
{
	...
	cout << "\n" << Get_Name_of_Entity(pMiner->Get_ID()) << ": 은행에 금을 맡겼다! (은행 잔고: " << pMiner->Get_Gold_in_Bank() << ")";
...
void Quench_Thirst::Execute(Miner* pMiner)
{
	if (pMiner->is_Thirsty())
	{
		...
		{
			cout << "\n" << Get_Name_of_Entity(pMiner->Get_ID()) << ": 위스키를 한잔 사서 마셨다. (소지금: " << pMiner->Get_Gold_Pocket() << ", 은행 잔고: " << pMiner->Get_Gold_in_Bank() << ")";
...
void Relieve_Hunger::Execute(Miner* pMiner)
{
	if (pMiner->is_Hungry())
	{
		...
		{
			cout << "\n" << Get_Name_of_Entity(pMiner->Get_ID()) << ": (꺼억!) 밥을 사먹었다. (소지금: " << pMiner->Get_Gold_Pocket() << ", 은행 잔고: " << pMiner->Get_Gold_in_Bank() << ")";
